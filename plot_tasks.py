...
df = pd.read_csv("tasks_searchLocalBestImprovement_m10_r1.500000_exec1.csv")

# Agrupar tarefas por máquina
grouped = df.groupby("Machine")["TaskProcessingTime"].apply(list)
task_counts = df.groupby("Machine")["TaskProcessingTime"].count()  # Contagem de tarefas por máquina

# Criar o gráfico de barras empilhadas
fig, ax = plt.subplots(figsize=(12, 7), dpi=150)

# Gerar uma paleta de cores mais suaves
np.random.seed(42)
colors = plt.cm.get_cmap("Set2", len(df))

# Variável para rastrear a posição acumulada dentro da barra
bottoms = {m: 0 for m in grouped.index}

bar_width = 0.6  # Ajustando a largura para criar espaçamento entre máquinas
...
for index, row in df.iterrows():
    ...
    rects = ax.bar(machine, task_time, 
                   ...
                   edgecolor="black",  # Adiciona borda preta para separar melhor as tarefas
                   bottom=bottoms[machine], width=bar_width)

    # Adicionando texto dentro das barras para exibir o tempo de cada tarefa
    for rect in rects:
    ...
    # Atualiza a posição acumulada para a próxima tarefa
...
for machine, total_time in bottoms.items():
    ax.text(machine, total_time + 5, f"Makespan: {int(total_time)}", 
            ha='center', fontsize=11, fontweight='bold', bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
...
ax.set_xlabel("Máquinas", fontsize=13, fontweight="bold")
...
# Ajustar limites do eixo Y para melhor espaçamento
ax.set_ylim([0, max(bottoms.values()) * 1.15])

# Ajuste do grid para ficar mais sutil
